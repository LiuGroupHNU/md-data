...
def check_switch_range(davg, dstd):
    ...
    elif "train_attr.min_nbor_dist" in mdpu_cfg.weight.keys():
        if mdpu_cfg.weight["train_attr.min_nbor_dist"] < 1e-6:
            min_dist = rmin
        else:
            min_dist = mdpu_cfg.weight["train_attr.min_nbor_dist"]
    else:
        min_dist = rmin
    ...
    if (davg is not None) or (dstd is not None):
        ...
        mdpu_cfg.dscp["dmin"] = min_dist
        mdpu_cfg.get_s_range(davg, dstd)


def build_op_descriptor():
    r"""Replace se_a.py/DescrptSeA/build."""
    if mdpu_cfg.quantize_descriptor:
        return op_module.prod_env_mat_a_mix_mdpu_quantize
    else:
...
def descrpt2r4(inputs, atype):
    ...
    table = GLOBAL_NP_FLOAT_PRECISION(
        np.concatenate([mdpu_cfg.map["s"][0], mdpu_cfg.map["h"][0]], axis=1)
    )
    table_grad = GLOBAL_NP_FLOAT_PRECISION(
        np.concatenate(
            [mdpu_cfg.map["s_grad"][0], mdpu_cfg.map["h_grad"][0]],
            axis=1,
        )
    )
    table_info = mdpu_cfg.map["cfg_u2s"]
    table_info = np.array([np.float64(v) for vs in table_info for v in vs])
    table_info = GLOBAL_NP_FLOAT_PRECISION(table_info)

    s_h = op_module.map_flt_mdpu(u, table, table_grad, table_info)
    ...
    with tf.variable_scope("s_s", reuse=True):
        s = op_module.flt_mdpu(s)
        log.debug("#s_s: %s", s)
        s = tf.ensure_shape(s, [None, 1])

    with tf.variable_scope("h_s", reuse=True):
        h = op_module.flt_mdpu(h)
        log.debug("#h_s: %s", h)
        h = tf.ensure_shape(h, [None, 1])
    # davg and dstd
    # davg = mdpu_cfg.map["davg"]  # is_zero
    dstd_inv = mdpu_cfg.map["dstd_inv"]
    ...
    s = op_module.mul_flt_mdpu(std_inv_s, tf.reshape(s, [-1, NIDP]))
    h = op_module.mul_flt_mdpu(std_inv_h, tf.reshape(h, [-1, NIDP]))
...
def filter_lower_R42GR(inputs_i, atype, nei_type_vec):
    ...
    ntype = mdpu_cfg.dscp["ntype"]
    NIDP = mdpu_cfg.dscp["NIDP"]
    two_embd_value = mdpu_cfg.map["gt"]
    # print(two_embd_value)

    # copy
    inputs_reshape = op_module.flt_mdpu(inputs_reshape)
    inputs_reshape = tf.ensure_shape(inputs_reshape, [None, 4])

    inputs_reshape, inputs_reshape2 = op_module.copy_flt_mdpu(inputs_reshape)
    inputs_reshape = tf.ensure_shape(inputs_reshape, [None, 4])
    inputs_reshape2 = tf.ensure_shape(inputs_reshape2, [None, 4])
    # s2G
    s = tf.reshape(tf.slice(inputs_reshape, [0, 0], [-1, 1]), [-1, 1])
    table = GLOBAL_NP_FLOAT_PRECISION(mdpu_cfg.map["g"][0])
    table_grad = GLOBAL_NP_FLOAT_PRECISION(mdpu_cfg.map["g_grad"][0])
    table_info = mdpu_cfg.map["cfg_s2g"]
    table_info = np.array([np.float64(v) for vs in table_info for v in vs])
    table_info = GLOBAL_NP_FLOAT_PRECISION(table_info)
    G = op_module.map_flt_mdpu(s, table, table_grad, table_info)
    G = tf.ensure_shape(G, [None, 1, M1])
    with tf.variable_scope("g_s", reuse=True):
        G = op_module.flt_mdpu(G)
        log.debug("#g_s: %s", G)
        G = tf.ensure_shape(G, [None, 1, M1])
    # t2G
    atype_expand = tf.reshape(atype, [-1, 1])
    ...
    with tf.variable_scope("g_t", reuse=True):
        two_embd = op_module.flt_mdpu(two_embd)
        log.debug("#g_t: %s", two_embd)
        two_embd = tf.ensure_shape(two_embd, [None, M1])
    # G_s, G_t -> G
    G = tf.reshape(G, [-1, M1])
    G = op_module.mul_flt_mdpu(G, two_embd)
    ...
    GR = op_module.matmul_flt2fix_mdpu(
...
def filter_GR2D(xyz_scatter_1):
    ...
    NIX = mdpu_cfg.dscp["NIX"]
    M1 = mdpu_cfg.dscp["M1"]
    M2 = mdpu_cfg.dscp["M2"]
    NBIT_DATA_FL = mdpu_cfg.nbit["NBIT_FIXD_FL"]
    ...
    if mdpu_cfg.quantize_descriptor:
        ...
        with tf.variable_scope("gr", reuse=True):
            xyz_scatter_1 = op_module.flt_mdpu(xyz_scatter_1)
        ...
        result = op_module.matmul_flt_mdpu(
        ...
        with tf.variable_scope("d", reuse=True):
            result = op_module.flt_mdpu(result)
            log.debug("#d: %s", result)
            result = tf.ensure_shape(result, [None, M1 * M2])

        result = op_module.quantize_mdpu(result, 0, NBIT_DATA_FL, NBIT_DATA_FL, -1)
        result = tf.ensure_shape(result, [None, M1 * M2])
    else:
