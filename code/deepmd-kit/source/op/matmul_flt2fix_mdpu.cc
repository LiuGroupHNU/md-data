...
class MatmulFlt2fixMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit MatmulFlt2fixMdpuOp(OpKernelConstruction* context)
      : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                     \
  REGISTER_KERNEL_BUILDER(                                                  \
      Name("MatmulFlt2fixMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MatmulFlt2fixMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
