...
class CopyFltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit CopyFltMdpuOp(OpKernelConstruction* context) : OpKernel(context){};
...
#define REGISTER_CPU(T)                                               \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("CopyFltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      CopyFltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
