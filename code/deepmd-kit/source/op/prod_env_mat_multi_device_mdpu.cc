...
REGISTER_OP("ProdEnvMatAMdpuQuantize")
...
// ProdEnvMatAMixMdpu
REGISTER_OP("ProdEnvMatAMixMdpuQuantize")
    .Attr("T: {float, double} = DT_DOUBLE")
    .Input("coord: T")       // atomic coordinates
    .Input("type: int32")    // atomic type
    .Input("natoms: int32")  // local atomic number; each type atomic number
...
/*
//==================================================
  ProdEnvMatAMdpuQuantizeOp
//==================================================
*/

template <typename Device, typename FPTYPE>
class ProdEnvMatAMdpuQuantizeOp : public OpKernel {
 public:
  explicit ProdEnvMatAMdpuQuantizeOp(OpKernelConstruction* context)
      : OpKernel(context) {
    float nloc_f, nall_f;
...
/*
//==================================================
  ProdEnvMatAMixMdpuQuantizeOp
//==================================================
*/

template <typename Device, typename FPTYPE>
class ProdEnvMatAMixMdpuQuantizeOp : public OpKernel {
 public:
  explicit ProdEnvMatAMixMdpuQuantizeOp(OpKernelConstruction* context)
      : OpKernel(context) {
    float nloc_f, nall_f;
    OP_REQUIRES_OK(context, context->GetAttr("rcut_a", &rcut_a));
    OP_REQUIRES_OK(context, context->GetAttr("rcut_r", &rcut_r));
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(Name("ProdEnvMatAMdpuQuantize")           \
                              .Device(DEVICE_CPU)                    \
                              .TypeConstraint<T>("T"),               \
                          ProdEnvMatAMdpuQuantizeOp<CPUDevice, T>); \
  REGISTER_KERNEL_BUILDER(Name("ProdEnvMatAMixMdpuQuantize")        \
                              .Device(DEVICE_CPU)                    \
                              .TypeConstraint<T>("T"),               \
                          ProdEnvMatAMixMdpuQuantizeOp<CPUDevice, T>);
