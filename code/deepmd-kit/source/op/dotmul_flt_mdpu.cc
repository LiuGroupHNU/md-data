...
class DotmulFltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit DotmulFltMdpuOp(OpKernelConstruction *context)
      : OpKernel(context){};
...
#define REGISTER_CPU(T)                                                 \
  REGISTER_KERNEL_BUILDER(                                              \
      Name("DotmulFltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      DotmulFltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
