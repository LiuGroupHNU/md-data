...
class MatmulFltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit MatmulFltMdpuOp(OpKernelConstruction *context) : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                 \
  REGISTER_KERNEL_BUILDER(                                              \
      Name("MatmulFltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MatmulFltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
