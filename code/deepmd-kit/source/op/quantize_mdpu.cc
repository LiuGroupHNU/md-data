...
class QuantizeMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit QuantizeMdpuOp(OpKernelConstruction* context) : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                \
  REGISTER_KERNEL_BUILDER(                                             \
      Name("QuantizeMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      QuantizeMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
