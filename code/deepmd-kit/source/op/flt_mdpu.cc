...
class FltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit FltMdpuOp(OpKernelConstruction* context) : OpKernel(context){};
...
#define REGISTER_CPU(T)                                           \
  REGISTER_KERNEL_BUILDER(                                        \
      Name("FltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      FltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
