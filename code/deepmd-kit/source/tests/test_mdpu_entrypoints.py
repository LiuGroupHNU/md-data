...
class TestMdpuEntrypointsV0(tf.test.TestCase):
    @pytest.mark.run(order=0)
    def test_mapt_cnn_v0(self):
        config_file = str(tests_path / "mdpu" / "ref" / "config_v0_cnn.npy")
        weight_file = str(tests_path / "mdpu" / "ref" / "weight_v0_cnn.npy")
        map_file = str(tests_path / "mdpu" / "out" / "map_v0_cnn.npy")
        # mapt
    ...
    def test_model_qnn_v0(self):
        ...
        jdata_cf["weight_file"] = str(
            tests_path / "mdpu" / "ref" / "weight_v0_cnn.npy"
        )
        jdata_cf["map_file"] = str(tests_path / "mdpu" / "out" / "map_v0_cnn.npy")
        jdata_cf["enable"] = True
        mdpu_cfg.init_from_jdata(jdata_cf)
        ...
        # get feed_dict
        crd_file = str(tests_path / "mdpu" / "ref" / "coord.npy")
        box_file = str(tests_path / "mdpu" / "ref" / "box.npy")
        type_file = str(tests_path / "mdpu" / "ref" / "type.raw")
...
class TestMdpuEntrypointsV1(tf.test.TestCase):
    @pytest.mark.run(order=0)
    def test_mapt_cnn_v1(self):
        config_file = str(tests_path / "mdpu" / "ref" / "config_v1_cnn.npy")
        weight_file = str(tests_path / "mdpu" / "ref" / "weight_v1_cnn.npy")
        map_file = str(tests_path / "mdpu" / "out" / "map_v1_cnn.npy")
        # mapt
        mapObj = MapTable(config_file, weight_file, map_file)
        mapt = mapObj.build_map()
        #
    ...
    def test_model_qnn_v1(self):
        ...
        jdata_cf["weight_file"] = str(
            tests_path / "mdpu" / "ref" / "weight_v1_cnn.npy"
        )
        jdata_cf["map_file"] = str(tests_path / "mdpu" / "out" / "map_v1_cnn.npy")
        jdata_cf["enable"] = True
        mdpu_cfg.init_from_jdata(jdata_cf)
        mdpu_cfg.init_train_mode("qnn")
        # build trainer
        ...
        # test freeze
        sess = self.test_session().__enter__()
        weight_file1 = str(tests_path / "mdpu" / "ref" / "weight_v1_cnn.npy")
        weight_file2 = str(tests_path / "mdpu" / "out" / "weight_v1_qnn.npy")
    ...
    def test_wrap_qnn_v1(self):
        tf.reset_default_graph()
        jdata = {}
        jdata["mdpu_config"] = str(tests_path / "mdpu" / "ref" / "config_v1_cnn.npy")
        jdata["mdpu_weight"] = str(tests_path / "mdpu" / "ref" / "weight_v1_cnn.npy")
        jdata["mdpu_map"] = str(tests_path / "mdpu" / "out" / "map_v1_cnn.npy")
        jdata["mdpu_model"] = str(tests_path / "mdpu" / "out" / "model_v1_qnn.pb")
        wrap(**jdata)
        # test
        data = FioBin().load(jdata["mdpu_model"])
