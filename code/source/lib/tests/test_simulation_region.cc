...
TEST_F(TestRegion, gpu) {
  ...
  mdpu::malloc_device_memory_sync(boxt_dev, region.boxt, 9);
  mdpu::malloc_device_memory_sync(rec_boxt_dev, region.rec_boxt, 9);
  mdpu::malloc_device_memory_sync(ref_rp_dev, ref_rp);
  mdpu::malloc_device_memory_sync(ref_ri_dev, ref_ri);
  region_dev.boxt = boxt_dev;
  region_dev.rec_boxt = rec_boxt_dev;
  // check volume
  double vol[1];
  double* vol_dev = NULL;
  mdpu::malloc_device_memory(vol_dev, 1);
  mdpu::volume_gpu(vol_dev, region_dev);
  mdpu::memcpy_device_to_host(vol_dev, vol, 1);
  EXPECT_LT(fabs(vol[0] - expected_vol), 1e-10);
  // check conversion between phys and inter coords.
  double ri[3];
  double* ri_dev = NULL;
  mdpu::malloc_device_memory(ri_dev, 3);
  mdpu::convert_to_inter_gpu(ri_dev, region_dev, ref_rp_dev);
  mdpu::memcpy_device_to_host(ri_dev, ri, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(ri[ii] - ref_ri[ii]), 1e-10);
  }
  double rp2[3];
  double* rp2_dev = NULL;
  mdpu::malloc_device_memory(rp2_dev, 3);
  mdpu::convert_to_phys_gpu(rp2_dev, region_dev, ri_dev);
  mdpu::memcpy_device_to_host(rp2_dev, rp2, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(rp2[ii] - ref_rp[ii]), 1e-10);
  }
  double rp[3];
  double* rp_dev = NULL;
  mdpu::malloc_device_memory(rp_dev, 3);
  mdpu::convert_to_phys_gpu(rp_dev, region_dev, ref_ri_dev);
  mdpu::memcpy_device_to_host(rp_dev, rp, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(rp[ii] - ref_rp[ii]), 1e-10);
  }
  double ri2[3];
  double* ri2_dev = NULL;
  mdpu::malloc_device_memory(ri2_dev, 3);
  mdpu::convert_to_inter_gpu(ri2_dev, region_dev, rp_dev);
  mdpu::memcpy_device_to_host(ri2_dev, ri2, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(ri2[ii] - ref_ri[ii]), 1e-10);
  }
  mdpu::delete_device_memory(boxt_dev);
  mdpu::delete_device_memory(rec_boxt_dev);
  mdpu::delete_device_memory(vol_dev);
  mdpu::delete_device_memory(ref_rp_dev);
  mdpu::delete_device_memory(ref_ri_dev);
  mdpu::delete_device_memory(ri_dev);
  mdpu::delete_device_memory(rp2_dev);
  mdpu::delete_device_memory(rp_dev);
  mdpu::delete_device_memory(ri2_dev);
...
TEST_F(TestRegion, gpu) {
  ...
  double* vol_dev = NULL;
  mdpu::malloc_device_memory(vol_dev, 1);
  mdpu::volume_gpu_rocm(vol_dev, region_dev);
  mdpu::memcpy_device_to_host(vol_dev, vol, 1);
  EXPECT_LT(fabs(vol[0] - expected_vol), 1e-10);
  // check conversion between phys and inter coords.
  double ri[3];
  double* ri_dev = NULL;
  mdpu::malloc_device_memory(ri_dev, 3);
  mdpu::convert_to_inter_gpu_rocm(ri_dev, region_dev, ref_rp_dev);
  mdpu::memcpy_device_to_host(ri_dev, ri, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(ri[ii] - ref_ri[ii]), 1e-10);
  }
  ...
  double* rp2_dev = NULL;
  mdpu::malloc_device_memory(rp2_dev, 3);
  mdpu::convert_to_phys_gpu_rocm(rp2_dev, region_dev, ri_dev);
  mdpu::memcpy_device_to_host(rp2_dev, rp2, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(rp2[ii] - ref_rp[ii]), 1e-10);
  }
  ...
  double* rp_dev = NULL;
  mdpu::malloc_device_memory(rp_dev, 3);
  mdpu::convert_to_phys_gpu_rocm(rp_dev, region_dev, ref_ri_dev);
  mdpu::memcpy_device_to_host(rp_dev, rp, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(rp[ii] - ref_rp[ii]), 1e-10);
  }
  ...
  double* ri2_dev = NULL;
  mdpu::malloc_device_memory(ri2_dev, 3);
  mdpu::convert_to_inter_gpu_rocm(ri2_dev, region_dev, rp_dev);
  mdpu::memcpy_device_to_host(ri2_dev, ri2, 3);
  for (int ii = 0; ii < 3; ++ii) {
    EXPECT_LT(fabs(ri2[ii] - ref_ri[ii]), 1e-10);
  }
