...
#include "prod_env_mat_mdpu.h"

class TestEnvMatAMdpu : public ::testing::Test {
 protected:
...
class TestEnvMatAMdpuShortSel : public ::testing::Test {
 protected:
  std::vector<double> posi = {12.83, 2.56, 2.18, 12.09, 2.87, 2.74,
                              00.25, 3.32, 1.68, 3.36,  3.00, 1.81,
                              3.51,  2.51, 2.60, 4.27,  3.22, 1.56};
...
/*  env_mat_a_mdpu_quantize_cpu is not same as env_mat_a.
remove some tests:
TEST_F(TestEnvMatAMdpu, orig_cpy)
TEST_F(TestEnvMatAMdpu, orig_pbc)
TEST_F(TestEnvMatAMdpu, orig_cpy_equal_pbc)
TEST_F(TestEnvMatAMdpu, orig_cpy_num_deriv)
*/

TEST_F(TestEnvMatAMdpu, cpu) {
...
/*  env_mat_a_mdpu_quantize_cpu is not same as env_mat_a.
remove some tests:
TEST_F(TestEnvMatAMdpu, cpu_equal_orig_cpy)
TEST_F(TestEnvMatAMdpu, cpu_num_deriv)
TEST_F(TestEnvMatAMdpuShortSel, orig_cpy)
TEST_F(TestEnvMatAMdpuShortSel, orig_pbc)
*/

TEST_F(TestEnvMatAMdpuShortSel, cpu) {
  std::vector<int> fmt_nlist_a, fmt_nlist_r;
  std::vector<double> env, env_deriv, rij_a;
  bool pbc = false;
  for (int ii = 0; ii < nloc; ++ii) {
...
TEST_F(TestEnvMatAMdpu, prod_cpu) {
...
TEST_F(TestEnvMatAMdpu, prod_cpu_equal_cpu) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
  int tot_nnei = 0;
  int max_nbor_size = 0;
  for (int ii = 0; ii < nlist_a_cpy.size(); ++ii) {
