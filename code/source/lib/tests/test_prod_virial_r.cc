 ...
 protected:
  ...
  void SetUp() override {
    ...
    for (int ii = 0; ii < nloc; ++ii) {
      ...
      mdpu::env_mat_r_cpu<double>(t_env, t_env_deriv, t_rij, posi_cpy,
...
TEST_F(TestProdVirialR, cpu) {
  ...
  int n_a_sel = nnei;
  mdpu::prod_virial_r_cpu<double>(&virial[0], &atom_virial[0], &net_deriv[0],
...
TEST_F(TestProdVirialR, gpu_cuda) {
  ...
  mdpu::malloc_device_memory_sync(virial_dev, virial);
  mdpu::malloc_device_memory_sync(atom_virial_dev, atom_virial);
  ...
  mdpu::prod_virial_r_gpu_cuda<double>(virial_dev, atom_virial_dev,
                                         net_deriv_dev, env_deriv_dev, rij_dev,
                                         nlist_dev, nloc, nall, nnei);

  mdpu::memcpy_device_to_host(virial_dev, virial);
  mdpu::memcpy_device_to_host(atom_virial_dev, atom_virial);
  mdpu::delete_device_memory(nlist_dev);
  mdpu::delete_device_memory(virial_dev);
  mdpu::delete_device_memory(atom_virial_dev);
...
TEST_F(TestProdVirialR, gpu_rocm) {
  ...
  mdpu::prod_virial_r_gpu_rocm<double>(virial_dev, atom_virial_dev,
                                         net_deriv_dev, env_deriv_dev, rij_dev,
                                         nlist_dev, nloc, nall, nnei);
