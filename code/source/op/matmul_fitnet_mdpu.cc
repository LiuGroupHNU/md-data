...
class MatmulFitnetMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit MatmulFitnetMdpuOp(OpKernelConstruction* context)
      : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                    \
  REGISTER_KERNEL_BUILDER(                                                 \
      Name("MatmulFitnetMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MatmulFitnetMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
