...
class AddFltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit AddFltMdpuOp(OpKernelConstruction* context) : OpKernel(context){};
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(                                           \
      Name("AddFltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      AddFltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
