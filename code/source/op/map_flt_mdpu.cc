...
void mul_flt_mdpu(T& y, T x1, T x2);

template <class T>  // float and double
void add_flt_mdpu(T& y, T x1, T x2);

//- register the operator
// prec = 2^n, so it doesn't need to match `T`
REGISTER_OP("MapFltMdpu")
...
class MapFltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit MapFltMdpuOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    for (ss = S - 1; ss >= 0; ss--) {
      ...
      for (ii = 0; ii < N * D; ii++) {
        ...
        for (jj = 0; jj < M; jj++) {
          ...
          mul_flt_mdpu(ytmp, a, xx);
          add_flt_mdpu(ytmp, b, ytmp);
          mul_flt_mdpu(ytmp, ytmp, xx);
          add_flt_mdpu(ytmp, c, ytmp);
          mul_flt_mdpu(ytmp, ytmp, xx);
          add_flt_mdpu(ytmp, d, ytmp);
          y[ii * M + jj] = ytmp;
        }  // jj
      }    // ii
    }      // ss
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(                                           \
      Name("MapFltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MapFltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
