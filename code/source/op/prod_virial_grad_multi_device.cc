 ...
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    for (int_64 kk = 0; kk < nframes; ++kk) {
      ...
      if (device == "GPU") {
#if GOOGLE_CUDA
        mdpu::prod_virial_grad_a_gpu_cuda(grad_net, grad, in_deriv, rij,
                                            nlist, nloc, nnei);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
        mdpu::prod_virial_grad_a_gpu_rocm(grad_net, grad, in_deriv, rij,
                                            nlist, nloc, nnei);
#endif  // TENSORFLOW_USE_ROCM
      } else if (device == "CPU") {
        mdpu::prod_virial_grad_a_cpu(grad_net, grad, in_deriv, rij, nlist,
                                       nloc, nnei);
      }
    }
  }
 ...
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    for (int_64 kk = 0; kk < nframes; ++kk) {
      ...
      if (device == "GPU") {
#if GOOGLE_CUDA
        mdpu::prod_virial_grad_r_gpu_cuda(grad_net, grad, in_deriv, rij,
                                            nlist, nloc, nnei);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
        mdpu::prod_virial_grad_r_gpu_rocm(grad_net, grad, in_deriv, rij,
                                            nlist, nloc, nnei);
#endif  // TENSORFLOW_USE_ROCM
      } else if (device == "CPU") {
        mdpu::prod_virial_grad_r_cpu(grad_net, grad, in_deriv, rij, nlist,
                                       nloc, nnei);
      }
    }
  }
