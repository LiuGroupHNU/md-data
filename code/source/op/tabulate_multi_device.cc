...
class TabulateFusionSeAOp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_a_gpu_cuda(descriptor, table, table_info, em_x,
                                            em, two_embed, nloc, nnei,
                                            last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_a_gpu_rocm(descriptor, table, table_info, em_x,
                                            em, two_embed, nloc, nnei,
                                            last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_a_cpu(descriptor, table, table_info, em_x, em,
...
class TabulateFusionSeAGradOp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_a_grad_gpu_cuda(
          dy_dem_x, dy_dem, table, table_info, em_x, em, two_embed, dy, nloc,
          nnei, last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_a_grad_gpu_rocm(
          dy_dem_x, dy_dem, table, table_info, em_x, em, two_embed, dy, nloc,
          nnei, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_a_grad_cpu(dy_dem_x, dy_dem, table, table_info,
...
class TabulateFusionSeAGradGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_a_grad_grad_gpu_cuda(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei, last_layer_size, is_sorted);
#endif  // GOOGLE_CUDA
#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_a_grad_grad_gpu_rocm(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei, last_layer_size, is_sorted);
#endif  // TENSORFLOW_USE_ROCM
      ...
      mdpu::tabulate_fusion_se_a_grad_grad_cpu(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei, last_layer_size, is_sorted);
    }
  }
...
class TabulateFusionSeTOp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_t_gpu_cuda(descriptor, table, table_info, em_x,
                                            em, nloc, nnei_i, nnei_j,
                                            last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_t_gpu_rocm(descriptor, table, table_info, em_x,
                                            em, nloc, nnei_i, nnei_j,
                                            last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_t_cpu(descriptor, table, table_info, em_x, em,
...
class TabulateFusionSeTGradOp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_t_grad_gpu_cuda(
          dy_dem_x, dy_dem, table, table_info, em_x, em, dy, nloc, nnei_i,
          nnei_j, last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_t_grad_gpu_rocm(
          dy_dem_x, dy_dem, table, table_info, em_x, em, dy, nloc, nnei_i,
          nnei_j, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_t_grad_cpu(dy_dem_x, dy_dem, table, table_info,
...
class TabulateFusionSeTGradGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_t_grad_grad_gpu_cuda(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei_i, nnei_j, last_layer_size);
#endif  // GOOGLE_CUDA
#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_t_grad_grad_gpu_rocm(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei_i, nnei_j, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
      OP_REQUIRES(context, (last_layer_size <= 1024),
      ...
      mdpu::tabulate_fusion_se_t_grad_grad_cpu(
          dz_dy, table, table_info, em_x, em, dz_dy_dem_x, dz_dy_dem, nloc,
          nnei_i, nnei_j, last_layer_size);
    }
  }
...
class TabulateFusionSeROp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_r_gpu_cuda(descriptor, table, table_info, em,
                                            nloc, nnei, last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_r_gpu_rocm(descriptor, table, table_info, em,
                                            nloc, nnei, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_r_cpu(descriptor, table, table_info, em, nloc,
                                       nnei, last_layer_size);
    }
  }
...
class TabulateFusionSeRGradOp : public OpKernel {
 public:
  ...
  void _Compute(OpKernelContext* context) {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_r_grad_gpu_cuda(
          dy_dem, table, table_info, em, dy, nloc, nnei, last_layer_size);
#endif  // GOOGLE_CUDA

#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_r_grad_gpu_rocm(
          dy_dem, table, table_info, em, dy, nloc, nnei, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      mdpu::tabulate_fusion_se_r_grad_cpu(dy_dem, table, table_info, em, dy,
                                            nloc, nnei, last_layer_size);
    }
  }
...
class TabulateFusionSeRGradGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (device == "GPU") {
#if GOOGLE_CUDA
      mdpu::tabulate_fusion_se_r_grad_grad_gpu_cuda(
          dz_dy, table, table_info, em, dz_dy_dem, nloc, nnei, last_layer_size);
#endif  // GOOGLE_CUDA
#if TENSORFLOW_USE_ROCM
      mdpu::tabulate_fusion_se_r_grad_grad_gpu_rocm(
          dz_dy, table, table_info, em, dz_dy_dem, nloc, nnei, last_layer_size);
#endif  // TENSORFLOW_USE_ROCM
      OP_REQUIRES(context, (last_layer_size <= 1024),
                  errors::InvalidArgument(
      ...
      mdpu::tabulate_fusion_se_r_grad_grad_cpu(
          dz_dy, table, table_info, em, dz_dy_dem, nloc, nnei, last_layer_size);
    }
  }
