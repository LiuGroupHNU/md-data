...
@ops.RegisterGradient("MatmulFltMdpu")
def _MatmulFltMdpuGrad(op, grad):
    x = op.inputs[0]
    w = op.inputs[1]
    ...
    modew = (normw >> 4) & 15
    if modex:
        dx = op_module.matmul_flt2fix_mdpu(grad, w_T, 23)
    else:
        dx = op_module.matmul_flt_mdpu(grad, w_T, normx, normw)
    if modew:
        dw = op_module.matmul_flt2fix_mdpu(x_T, grad, 23)
    else:
        dw = op_module.matmul_flt_mdpu(x_T, grad, 1, normx)
