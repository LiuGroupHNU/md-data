...
// New Activation Function of MDPU
// --------------------------------------------------------------------
/*
...
class Tanh4FltMdpuOp : public OpKernel {
 public:
  /// Constructor.
  explicit Tanh4FltMdpuOp(OpKernelConstruction* context) : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                \
  REGISTER_KERNEL_BUILDER(                                             \
      Name("Tanh4FltMdpu").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      Tanh4FltMdpuOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
