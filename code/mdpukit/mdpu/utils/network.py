...
def one_layer_wb(
    ...
    if mdpu_cfg.restore_fitting_net:
        # initializer
        w_initializer = get_constant_initializer(mdpu_cfg.weight, "matrix")
        b_initializer = get_constant_initializer(mdpu_cfg.weight, "bias")
    else:
...
def one_layer_t(
    ...
    NTAVC = mdpu_cfg.fitn["NTAVC"]
    if mdpu_cfg.restore_fitting_net:
        t_initializer = get_constant_initializer(mdpu_cfg.weight, "tweight")
    else:
...
def one_layer(
    ...
    is_layer = (mdpu_cfg.version == 1) and ("layer_0" in name)
    with tf.variable_scope(name, reuse=reuse):
        ...
        if mdpu_cfg.quantize_fitting_net:
            ...
            with tf.variable_scope("wxb", reuse=reuse):
                wxb = op_module.quantize_mdpu(wxb, 1, NBIT_DATA_FL, NBIT_DATA_FL, -1)
                wxb = tf.ensure_shape(wxb, [None, outputs_size])
            # actfun
            if activation_fn is not None:
                # set activation function as tanh4
                y = op_module.tanh4_flt_mdpu(wxb)
            else:
                y = wxb

            with tf.variable_scope("actfun", reuse=reuse):
                y = op_module.quantize_mdpu(y, 1, NBIT_DATA_FL, NBIT_DATA_FL, -1)
