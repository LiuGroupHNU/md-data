...
class MapTable:
    ...
    config_file
        input file name
        an .npy file containing the configuration information of MDPU model
    weight_file
        input file name
        an .npy file containing the weights of MDPU model
    map_file
        output file name
        an .npy file containing the mapping tables of MDPU model
    ...
    def __init__(self, config_file: str, weight_file: str, map_file: str):
        ...
        mdpu_cfg.init_from_jdata(jdata)

    def build_map(self):
        M = mdpu_cfg.dscp["M1"]
        if mdpu_cfg.version == 0:
            ndim = mdpu_cfg.dscp["ntype"]
        if mdpu_cfg.version == 1:
            ndim = 1
        ...
        log.info("MDPU: finish building mapping table")
        return self.map
    ...
    def mapping2(self, x, dic_map, cfgs):
        ...
        t_y = op_module.map_flt_mdpu(t_x, t_table, t_table_grad, t_table_info)
        sess = get_sess()
        #
        n = len(x)
        dic_val = {}
    ...
    def build_u2s(self, r2):
        ...
        rmax = mdpu_cfg.dscp["rcut"]
        dmin = mdpu_cfg.dscp["dmin"]

        min_dist = rmin
        if "train_attr.min_nbor_dist" in mdpu_cfg.weight.keys():
        ...
        if mdpu_cfg.version == 0:
            ...
            avg, std = get_normalize(mdpu_cfg.weight)
    ...
    def build_s2g(self, s):
        ...
        for tt2 in range(ntype):
            wbs = [get_filter_weight(mdpu_cfg.weight, tt2, ll) for ll in range(1, 5)]
    ...
    def run_s2g(self):
        ...
        smin = mdpu_cfg.dscp["smin"]
        smax = mdpu_cfg.dscp["smax"]
    ...
    def build_t2g(self):
        ...
        wbs = [get_type_embedding_weight(mdpu_cfg.weight, ll) for ll in range(1, 5)]
        ...
        wbs = [get_filter_type_weight(mdpu_cfg.weight, ll) for ll in range(1, 5)]
...
def mapt(
    *,
    mdpu_config: Optional[str] = "mdpu/config.npy",
    mdpu_weight: Optional[str] = "mdpu/weight.npy",
    mdpu_map: Optional[str] = "mdpu/map.npy",
    **kwargs,
):
    # build mapping table
    mapObj = MapTable(mdpu_config, mdpu_weight, mdpu_map)
    mapObj.build_map()
