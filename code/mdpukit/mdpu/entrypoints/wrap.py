...
from mdpukit.mdpu.utils.encode import (
    Encode,
)
...
class Wrap:
    ...
    the model file can be use to run the MDPU with lammps
    ...
    model_file
        output file name
        an .pb file containing the model using in the MDPU
    ...
    def wrap(self):
        ...
        for ii in range(len(datas)):
            ...
            if jdata_sys["debug"]:
                ...
                FioTxt().save("mdpu/wrap/h%s.txt" % (k), d)
            datas[ii] = d
        # update h & w of mdpu_cfg
        mdpu_cfg.size["NH_DATA"] = nhs
        mdpu_cfg.size["NW_DATA"] = nws
        mdpu_cfg.save(mdpu_cfg.config_file)
        ...
        log.info("MDPU: finish wrapping model file")
    ...
    def wrap_lut(self):
        ...
        NBIT_FLTE = nbit["NBIT_FLTE"]
        NBIT_FLTF = nbit["NBIT_FLTF"]
        NBIT_DATA = mdpu_cfg.nbit["NBIT_FIT_DATA"]
        NBIT_WXDB = mdpu_cfg.nbit["NBIT_FIT_WXDB"]
        NBIT_DATA_FL = mdpu_cfg.nbit["NBIT_FIT_DATA_FL"]
...
def wrap(
    *,
    ...
    mdpu_model: Optional[str] = "mdpu/model.pb",
    **kwargs,
):
    wrapObj = Wrap(mdpu_config, mdpu_weight, mdpu_map, mdpu_model)
    wrapObj.wrap()
