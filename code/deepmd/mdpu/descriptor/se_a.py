...
def build_op_descriptor():
    ...
    if mdpu_cfg.quantize_descriptor:
        return op_module.prod_env_mat_a_mdpu_quantize
    else:
    ...
    with tf.variable_scope("filter_type_all_x", reuse=True):
        ...
        for type_i in range(ntypes):
            ...
            table = GLOBAL_NP_FLOAT_PRECISION(
                np.concatenate(
                    [mdpu_cfg.map["s"][type_i], mdpu_cfg.map["h"][type_i]], axis=1
                )
            )
            table_grad = GLOBAL_NP_FLOAT_PRECISION(
                np.concatenate(
                    [mdpu_cfg.map["s_grad"][type_i], mdpu_cfg.map["h_grad"][type_i]],
                    axis=1,
                )
            )
            ...
            s_h_i = op_module.map_flt_mdpu(u_i, table, table_grad, table_info)
        ...
        Rs = s
        # Rxyz = h * rij
        Rxyz = op_module.mul_flt_mdpu(h, rij)
...
def filter_lower_R42GR(
    ...
    if mdpu_cfg.quantize_descriptor:
        ...
        # G
        table = GLOBAL_NP_FLOAT_PRECISION(mdpu_cfg.map["g"][type_i])
        table_grad = GLOBAL_NP_FLOAT_PRECISION(mdpu_cfg.map["g_grad"][type_i])
        ...
        if mdpu_cfg.restore_descriptor:
            trainable = False
            embedding_net_variables = {}
            for key in mdpu_cfg.weight.keys():
                if "filter_type" in key:
                    key2 = key.replace(".", "/")
                    embedding_net_variables[key2] = mdpu_cfg.weight[key]
